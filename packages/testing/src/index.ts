...
export class TestModule {
  ...
  static async create(moduleClass: any, options: any = {}): Promise<TestModule> {
  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      for (const [_, suite] of this.suites) {
        ...
        const results = await suite.run(this.beforeEachHooks, this.afterEachHooks);
...
// 🎯 Test Suite
class TestSuite {
  private tests: Array<{ description: string; fn: () => void | Promise<void> }> = [];
...
export class Expect {
  ...
  toHaveLength(expected: number): this {
    if (!this.actual?.length || this.actual.length !== expected) {
      throw new Error(`Expected length ${expected}, got ${this.actual?.length || 0}`);
    }
    return this;
  }
...
export class HttpClient {
  ...
  async put<T = any>(path: string, body: any = {}, options: any = {}): Promise<HttpResponse<T>> {
    const response = await fetch(`${this.baseUrl}${path}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json', ...options.headers },
      body: JSON.stringify(body),
...
export const beforeEach = (fn: () => void | Promise<void>) => runner.beforeEach(fn);
export const afterEach = (fn: () => void | Promise<void>) => runner.afterEach(fn);
