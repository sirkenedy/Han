...
export class InfoCommand {
  async execute() {
    try {
      ...
      console.log(boxen(
        chalk.blue.bold('📋 Han Framework Project Information'),
        {
          padding: 1,
          margin: 1,
          borderStyle: 'round',
      ...
      console.error(chalk.red('Failed to gather project information:'), error.message);
    }
  }
  ...
  private async getProjectInfo() {
    ...
    if (!await fs.pathExists(packageJsonPath)) {
      throw new Error('Not in a Node.js project directory (package.json not found)');
    }

    const packageJson = await fs.readJson(packageJsonPath);
    const isHanProject = packageJson.dependencies?.['han-prev-core'] ||
    ...
    return {
      ...
      hanFramework: isHanProject ? packageJson.dependencies?.['han-prev-core'] ||
                                   packageJson.devDependencies?.['han-prev-core'] ||
                                   packageJson.dependencies?.['@han-prev/core'] ||
                                   packageJson.devDependencies?.['@han-prev/core'] ||
                                   packageJson.dependencies?.['han-framework'] ||
                                   packageJson.devDependencies?.['han-framework'] : null,
  ...
  private displayProjectInfo(info: any) {
    ...
    if (info.hanFramework) {
      console.log(chalk.white(`Han Framework: ${chalk.green(info.hanFramework)}`));
    } else {
      console.log(chalk.yellow('⚠️  This is not a Han Framework project'));
    }

    console.log(chalk.white(`Dependencies: ${chalk.cyan(info.dependencies.length)}`));
    console.log(chalk.white(`Dev Dependencies: ${chalk.cyan(info.devDependencies.length)}`));

    if (Object.keys(info.scripts).length > 0) {
      console.log(chalk.blue.bold('\n📝 Available Scripts:'));
      Object.entries(info.scripts).forEach(([script, command]) => {
        console.log(chalk.white(`  ${chalk.cyan(script)}: ${chalk.gray(command)}`));
      });
    }
  }
  ...
  private displayFrameworkInfo() {
    ...
    console.log(chalk.white(`Repository: ${chalk.cyan('https://github.com/your-org/han-framework')}`));
    console.log(chalk.white(`Documentation: ${chalk.cyan('https://han-framework.dev')}`));
    ...
    commands.forEach(({ cmd, desc }) => {
      console.log(chalk.white(`  ${chalk.cyan(cmd.padEnd(25))} ${chalk.gray(desc)}`));
    });
  }
}